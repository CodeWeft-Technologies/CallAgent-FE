...
export const LaserFlow = ({
  ...
  canvas.addEventListener('pointermove', onMove, { passive: true })
    ...
    canvas.addEventListener('webglcontextlost', onCtxLost as any, false)
    canvas.addEventListener('webglcontextrestored', onCtxRestored as any, false)
  ...
  const avgFps = samples.reduce((a: number, b: number) => a + b, 0) / samples.length
      ...
      if (avgFps < lowerThresh) {
        next = clampV(currentDprRef.current * 0.9, dprFloor, base)
      } else if (avgFps > upperThresh && currentDprRef.current < base) {
        next = clampV(currentDprRef.current * 1.05, dprFloor, base)
      }
  ...
  return <div ref={mountRef} className={`w-full h-full relative ${className || ''}`} style={style as CSSProperties} />
}
