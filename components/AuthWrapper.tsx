...
const AuthWrapper = React.memo<AuthWrapperProps>(({ children }) => {
  ...
  // Allow access to landing page, login page, register page, pricing page, features page and admin-login page without authentication
  if (pathname === '/' || pathname === '/login' || pathname === '/register' || pathname === '/pricing' || pathname === '/features' || pathname === '/admin-login') {
    return <>{children}</>
  }
